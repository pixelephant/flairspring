...
class CheckoutController < ApplicationController
  ...
  def payment
    ...
    if current_cart.total == 0
      ...
      if current_user.blank?
        ...
        shipping_address_text = data[:shipping_name].to_s + ": " + data[:shipping_zip].to_s + " " + data[:shipping_city].to_s + ", " + data[:shipping_additional]

        if data[:same] == 1
          invoice_address_text = shipping_address_text
        else
          invoice_address_text = data[:billing_name].to_s + ": " + data[:billing_zip].to_s + " " + data[:billing_city].to_s + ", " + data[:billing_additional]
        end
      else
        @user = current_user
        @user.title_name = data[:title_name]
      ...
      @order = Order.new(:shipping_address_id => (shipping ||= nil), :invoice_address_id => (billing ||= nil), :user_id => user_id, :status => "Feldolgozás alatt", :estimated_date => ((Time.now + 7.days).to_date), :shipping_address_text => shipping_address_text, :invoice_address_text => invoice_address_text, :gift => data[:gift_wrap], :personal_pickup => data[:personal_pickup], :title_name => data[:title_name], :first_name => data[:first_name], :last_name => data[:last_name], :phone => data[:phone], :email => data[:email])
      ...
      if @order.save
        ...
        session[:lazy_reg_order_id] = @order.id
  ...
  def lazy_registration
    respond_to do |format|
      ...
      if session[:lazy_reg_order_id].blank?
        format.json { render :json => {:status => "false"} }
      else
        session[:checkout_user] = nil

        order = Order.find(session[:lazy_reg_order_id])

        session[:lazy_reg_order_id] = nil

        user = User.create(:email => order.email, :password => order.email)
        user.save!
        user.title_name = order.title_name
